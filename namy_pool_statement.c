...
#	define pdo_namy_pool_stmt_execute_prepared(stmt) pdo_namy_pool_stmt_execute_prepared_mysqlnd(stmt TSRMLS_CC)
#	define pdo_free_bound_result(res) zval_dtor(res.zv)
#	define pdo_namy_pool_stmt_close(stmt) mysqlnd_stmt_close(stmt, 0)
#else
#	define pdo_namy_pool_stmt_execute_prepared(stmt) pdo_namy_pool_stmt_execute_prepared_libmysql(stmt TSRMLS_CC)
#	define pdo_free_bound_result(res) efree(res.buffer)
#	define pdo_namy_pool_stmt_close(stmt) mysql_stmt_close(stmt)
#endif



static int pdo_namy_pool_stmt_dtor(pdo_stmt_t *stmt TSRMLS_DC) /* {{{ */
{
	pdo_namy_pool_stmt *S = (pdo_namy_pool_stmt*)stmt->driver_data;

	PDO_DBG_ENTER("pdo_namy_pool_stmt_dtor");
	...
	if (S->stmt) {
		pdo_namy_pool_stmt_close(S->stmt);
		S->stmt = NULL;
	}
...
static void pdo_namy_pool_stmt_set_row_count(pdo_stmt_t *stmt TSRMLS_DC) /* {{{ */
{
	long row_count;
	pdo_namy_pool_stmt *S = stmt->driver_data;
...
#ifdef HAVE_MYSQL_STMT_PREPARE
static int pdo_namy_pool_stmt_execute_prepared_libmysql(pdo_stmt_t *stmt TSRMLS_DC) /* {{{ */
{
	pdo_namy_pool_stmt *S = stmt->driver_data;
	pdo_namy_pool_db_handle *H = S->H;

	PDO_DBG_ENTER("pdo_namy_pool_stmt_execute_prepared_libmysql");
	...
	pdo_namy_pool_stmt_set_row_count(stmt TSRMLS_CC);
	PDO_DBG_RETURN(1);
}
/* }}} */
#endif
...
#ifdef PDO_USE_MYSQLND
static int pdo_namy_pool_stmt_execute_prepared_mysqlnd(pdo_stmt_t *stmt TSRMLS_DC) /* {{{ */
{
	pdo_namy_pool_stmt *S = stmt->driver_data;
	pdo_namy_pool_db_handle *H = S->H;
	int i;
	
	PDO_DBG_ENTER("pdo_namy_pool_stmt_execute_prepared_mysqlnd");
...
static int pdo_namy_pool_stmt_execute(pdo_stmt_t *stmt TSRMLS_DC) /* {{{ */
{
	pdo_namy_pool_stmt *S = (pdo_namy_pool_stmt*)stmt->driver_data;
	pdo_namy_pool_db_handle *H = S->H;
	my_ulonglong row_count;
	PDO_DBG_ENTER("pdo_namy_pool_stmt_execute");
	PDO_DBG_INF_FMT("stmt=%p", S->stmt);

#if HAVE_MYSQL_STMT_PREPARE || PDO_USE_MYSQLND
	if (S->stmt) {
		PDO_DBG_RETURN(pdo_namy_pool_stmt_execute_prepared(stmt));
	}
#endif
...
static int pdo_namy_pool_stmt_next_rowset(pdo_stmt_t *stmt TSRMLS_DC) /* {{{ */
{
#if HAVE_MYSQL_NEXT_RESULT || PDO_USE_MYSQLND
	pdo_namy_pool_stmt *S = (pdo_namy_pool_stmt*)stmt->driver_data;
	pdo_namy_pool_db_handle *H = S->H;
	long row_count;
	int ret;
	PDO_DBG_ENTER("pdo_namy_pool_stmt_next_rowset");
	PDO_DBG_INF_FMT("stmt=%p", S->stmt);
...
static int pdo_namy_pool_stmt_param_hook(pdo_stmt_t *stmt, struct pdo_bound_param_data *param,
		enum pdo_param_event event_type TSRMLS_DC) /* {{{ */
{
#ifndef PDO_USE_MYSQLND
	PDO_NAMY_POOL_PARAM_BIND *b;
#endif
#if HAVE_MYSQL_STMT_PREPARE || PDO_USE_MYSQLND
	pdo_namy_pool_stmt *S = (pdo_namy_pool_stmt*)stmt->driver_data;	

	PDO_DBG_ENTER("pdo_namy_pool_stmt_param_hook");
	PDO_DBG_INF_FMT("stmt=%p", S->stmt);
...
#else
				b = (PDO_NAMY_POOL_PARAM_BIND*)param->driver_data;
				*b->is_null = 0; 
				if (PDO_PARAM_TYPE(param->param_type) == PDO_PARAM_NULL ||  
						Z_TYPE_P(param->parameter) == IS_NULL) { 
					*b->is_null = 1; 
...
static int pdo_namy_pool_stmt_fetch(pdo_stmt_t *stmt,
	enum pdo_fetch_orientation ori, long offset TSRMLS_DC) /* {{{ */
{
	...
	PDO_DBG_ENTER("pdo_namy_pool_stmt_fetch");
	PDO_DBG_INF_FMT("stmt=%p", S->stmt);
	if (S->stmt) {
...
static int pdo_namy_pool_stmt_describe(pdo_stmt_t *stmt, int colno TSRMLS_DC) /* {{{ */
{
	pdo_namy_pool_stmt *S = (pdo_namy_pool_stmt*)stmt->driver_data;
	struct pdo_column_data *cols = stmt->columns;
	int i;

	PDO_DBG_ENTER("pdo_namy_pool_stmt_describe");
	PDO_DBG_INF_FMT("stmt=%p", S->stmt);
	if (!S->result) {
		PDO_DBG_RETURN(0);
	}
...
static int pdo_namy_pool_stmt_get_col(pdo_stmt_t *stmt, int colno, char **ptr, unsigned long *len, int *caller_frees TSRMLS_DC) /* {{{ */
{
	pdo_namy_pool_stmt *S = (pdo_namy_pool_stmt*)stmt->driver_data;

	PDO_DBG_ENTER("pdo_namy_pool_stmt_get_col");
	PDO_DBG_INF_FMT("stmt=%p", S->stmt);
	if (!S->result) {
		PDO_DBG_RETURN(0);
	}
...
{
#define PDO_NAMY_POOL_NATIVE_TYPE_NAME(x)	case FIELD_TYPE_##x: return #x;

    switch (type) {
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(STRING)
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(VAR_STRING)
#ifdef MYSQL_HAS_TINY
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(TINY)
#endif
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(SHORT)
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(LONG)
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(LONGLONG)
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(INT24)
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(FLOAT)
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(DOUBLE)
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(DECIMAL)
#ifdef FIELD_TYPE_NEWDECIMAL
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(NEWDECIMAL)
#endif
#ifdef FIELD_TYPE_GEOMETRY
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(GEOMETRY)
#endif
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(TIMESTAMP)
#ifdef MYSQL_HAS_YEAR
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(YEAR)
#endif
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(SET)
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(ENUM)
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(DATE)
#ifdef FIELD_TYPE_NEWDATE
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(NEWDATE)
#endif
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(TIME)
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(DATETIME)
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(TINY_BLOB)
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(MEDIUM_BLOB)
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(LONG_BLOB)
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(BLOB)
        PDO_NAMY_POOL_NATIVE_TYPE_NAME(NULL)
        default:
            return NULL;
    }
#undef PDO_NAMY_POOL_NATIVE_TYPE_NAME
} /* }}} */

static int pdo_namy_pool_stmt_col_meta(pdo_stmt_t *stmt, long colno, zval *return_value TSRMLS_DC) /* {{{ */
{
	pdo_namy_pool_stmt *S = (pdo_namy_pool_stmt*)stmt->driver_data;
	const MYSQL_FIELD *F;
	zval *flags;
	...
	PDO_DBG_ENTER("pdo_namy_pool_stmt_col_meta");
	PDO_DBG_INF_FMT("stmt=%p", S->stmt);
	if (!S->result) {
...
static int pdo_namy_pool_stmt_cursor_closer(pdo_stmt_t *stmt TSRMLS_DC) /* {{{ */
{
	pdo_namy_pool_stmt *S = (pdo_namy_pool_stmt*)stmt->driver_data;

	PDO_DBG_ENTER("pdo_namy_pool_stmt_cursor_closer");
	PDO_DBG_INF_FMT("stmt=%p", S->stmt);
	if (S->result) {
		mysql_free_result(S->result);
		S->result = NULL;
...
struct pdo_stmt_methods namy_pool_stmt_methods = {
	pdo_namy_pool_stmt_dtor,
	pdo_namy_pool_stmt_execute,
	pdo_namy_pool_stmt_fetch,
	pdo_namy_pool_stmt_describe,
	pdo_namy_pool_stmt_get_col,
	pdo_namy_pool_stmt_param_hook,
	NULL, /* set_attr */
	NULL, /* get_attr */
	pdo_namy_pool_stmt_col_meta,
	pdo_namy_pool_stmt_next_rowset,
	pdo_namy_pool_stmt_cursor_closer
};
